...
let
	w = round(12.8Plots.cm / Plots.px)
	h = round(7.9Plots.cm / Plots.px)
...
begin
	@df df violin(string.(:la), :tex)
	@df df dotplot!(string.(:la), :tex)
end

# ╔═╡ 5e74a631-7eb0-40be-9a0a-05c2f6c50e12
begin
	@df df scatter(:la, :tex, group=:isd)
end

# ╔═╡ 582dd0f2-bfb3-4971-a5ac-b77f686811fc
plt_lambda_L = let
	plt = @df df groupedboxplot(string.(:la), :L, group=(:isd))
	xlabel!(L"$\lambda$")
	ylabel!(L"$L^*$")
